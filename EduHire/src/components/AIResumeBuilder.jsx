...
const AIResumeBuilder = ({ onSectionChange }) => {
  ...
  // Load profile data and update resume data (await async getter to avoid Promise misuse)
  useEffect(() => {
    let mounted = true;
    (async () => {
      try {
        ...
        if (profile.personal) {
          setResumeData(prev => ({
            ...
            personalInfo: {
              ...
              email: profile.personal.email || prev.personalInfo.email,
              phone: profile.personal.phone || prev.personalInfo.phone,
              location: profile.personal.location || prev.personalInfo.location,
              linkedin: profile.personal.linkedin || prev.personalInfo.linkedin
            }
          }));
        }
        ...
        if (profile.education && profile.education.length > 0) {
          ...
          setResumeData(prev => ({
            ...prev,
            education: {
              degree: latestEducation.degree || prev.education.degree,
              institution: latestEducation.institution || prev.education.institution,
              graduation: latestEducation.endDate || prev.education.graduation,
              gpa: latestEducation.gpa || prev.education.gpa
            }
          }));
        }
        ...
        console.error('Failed to load current user in AIResumeBuilder:', err);
      }
    })();
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center space-x-4">
              ...
              <div className="flex items-center space-x-4">
                <Button onClick={() => setPreviewMode(!previewMode)} leftIcon={<Eye size={16} />} variant={previewMode ? 'secondary' : 'ghost'}>
                  {previewMode ? 'Edit' : 'Preview'}
                </Button>
                <Button onClick={downloadResume} leftIcon={<Download size={16} />} variant="secondary">
                  Download
                </Button>
              </div>
          </div>
          ...
          {!previewMode && (
            ...
            >
                ...
                <div className="grid grid-cols-2 gap-4">
                      {templates.map((template) => (
                        <button
                          ...
                          className={`template-preview transition-all duration-200 ${
                            selectedTemplate === template.id
                              ? 'ring-2 ring-offset-2 ring-brand-light scale-105'
                              : 'hover:scale-102'
                          }`}
                        >
                          <div className={`w-full h-12 ${template.color} rounded mb-3`} />
                          <div className="flex items-center justify-between w-full">
                            <span className="text-sm font-medium text-[#1E293B]">{template.name}</span>
                            {selectedTemplate === template.id && <span className="text-xs text-gray-500">Selected</span>}
                          </div>
                        </button>
                      ))}
                </div>
              ...
              <div className="bg-white rounded-xl shadow-sm p-6">
                ...
                className="mt-4 flex items-center space-x-2 btn-primary disabled:opacity-50"
              >
                  {isGenerating ? (
                    <>
